...
class Admin extends CI_Controller {
	...
	public function assignEBDesignationMember($id)
	{
		$this->check_login();
		$data['eb_category']=$this->model->getEBCategory();
		$data['eb_cell']=$this->model->getEBCell();
		...
		$this->load->view('admin/header');
		$this->load->view('admin/sidebar');
		$this->load->view('admin/assign_designation_member',$data);
		$this->load->view('admin/footer');
	}
	...
	public function assignEBDesignationFinalMember()
	{
		$this->check_login();
		$eb_cid=$this->input->post('eb_cid');
		$eb_scid=$this->input->post('eb_scid');
  ...
  $object_writer->save('php://output');
		...
		public function social_issue()
		{
			...
			$this->load->view('admin/header');
			$this->load->view('admin/sidebar');
			$this->load->view('admin/social_issue',$data);
			$this->load->view('admin/footer');
		}
		//16 Aug, 2021
		...
		{
		    if($this->model->deleteBlogCategory($cat_id)):
		$this->session->set_flashdata('msg', "Blog Category deleted successfully."); 
		    redirect(base_url().'admin/blog_category');
		else:
			$this->session->set_flashdata('msg', "Something went wrong."); 
		    redirect(base_url().'admin/blog_category');
		...
		{
		    $data['blog_category']=$this->model->getBlogCategory($cat_id);
		    $this->load->view('admin/header');
			$this->load->view('admin/sidebar');
			$this->load->view('admin/edit_blog_category',$data);
			$this->load->view('admin/footer');
		}
		public function updateBlogCategory($cat_id)
		{
		 $cat_name=$this->input->post('cat_name');
		 $data=array('cat_name'=>$cat_name);
		 if($this->model->updateBlogCategory($data,$cat_id)):
